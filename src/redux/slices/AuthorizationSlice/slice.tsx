...
export const fetchingLogin = createAsyncThunk<
  FetchAuthorizationType,
  FetchLoginParams
>("authorization/fetchLogin", async (params) => {
  const { email, password } = params;
  const { data } = await authAxios.post<FetchAuthorizationType>(
    "/auth/login",
    {
      email,
      password,
...
export const fetchingRegister = createAsyncThunk<
  FetchAuthorizationType,
  FetchRegisterParams
>("authorization/fetchRegister", async (params) => {
...
export const AuthorizationSlice = createSlice({
  ...
  reducers: {
    ...
    changeCurrentForm(state, action: PayloadAction<string>) {
